...
class _StudentListWidgetState extends ConsumerState<StudentListWidget> {
      ...
      child: Column(
        children: [
          AppSearchBar(
            ...
            hintText: 'Search by name, ID, or roll...',
  ...
  Widget _buildStudentList(BuildContext context, AsyncSnapshot<List<Student>> snapshot, List<Student> students, bool isEmpty) {
    ...
    if (isEmpty) return const AppEmptyState(title: 'No students found', description: 'Add your first student to get started.');
  ...
  Future<void> _handleImport() async {
    ...
    try {
      ...
      if (data.isNotEmpty) {
        ...
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Imported ${data.length} students.')));
      }
    } catch (e) {
